...
public class HungarianAlgorithm {
        ...
        for (double[] array1 : array) {
            for (int j = 0; j < array1.length; j++) {
                if (randomMethod.equals("random")) {
                    array1[j] = generator.nextDouble();
                }
                if (randomMethod.equals("gaussian")) {
                    array1[j] = generator.nextGaussian() / 4; //range length to 1.
                    if (array1[j] > 0.5) {
                        array1[j] = 0.5;
                    } //eliminate outliers.
                    if (array1[j] < -0.5) {
                    ...
                    array1[j] = array1[j] + 0.5; //make elements positive.
                }
            }
        }
    }
    ...
    //**********************************//
    public static int[][] hgAlgorithm(double[][] array, SumType sumType) {
        double[][] cost = copyOf(array);	//Create the cost matrix

        if (sumType == SumType.MAX) //Then array is weight array. Must change to cost.
        {
...
//            System.out.printf("array(%d,%d) = %.2f\n", assignment1[0] + 1, assignment1[1] + 1, array[assignment1[0]][assignment1[1]]);

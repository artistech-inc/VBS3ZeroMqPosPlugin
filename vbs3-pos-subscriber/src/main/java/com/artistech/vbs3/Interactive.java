...
//                UdpBroadcaster.UDP_PORT = Integer.parseInt(cmd.getOptionValue("p"));
        ...
        try (Socket requester = context2.socket(ZMQ.REQ)) {
            requester.connect("tcp://192.168.1.34:5555");
            System.out.println("launch and connect client.");
            String readLine = in.nextLine();
            while (!"exit".equals(readLine)) {
                Vbs3Protos.Command cmd = Vbs3Protos.Command.newBuilder().setCmd(readLine).build();
                requester.send(cmd.toByteArray(), 0);
